export class roleFighter {
    ...
    guard(creep: Creep, /*targetName: String, targetType: String */) {
        ...
        let closestEnemy: Creep = creep.pos.findClosestByPath(FIND_HOSTILE_CREEPS);
    ...
    invade(creep: Creep) {
        ...
        if (targetRoom == undefined || targetRoom == null) {
            ...
            if (creep.room.name != targetRoom) {
                ...
                //creep.moveTo(flag);
                //return;
                let closestSpawn: Spawn = creep.pos.findClosestByPath(FIND_HOSTILE_SPAWNS, { algorithm: 'astar' });
